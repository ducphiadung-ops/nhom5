...
<head>
    <meta charset="UTF-8">
    <title>Quản lý Pin - Skycomputer</title>
    ...
    <style>
        ...
        .badge-active { background-color: #dcfce7; color: #16a34a; padding: 6px 12px; border-radius: 6px; font-weight: 500; font-size: 12px;}
        .badge-inactive { background-color: #fee2e2; color: #dc2626; padding: 6px 12px; border-radius: 6px; font-weight: 500; font-size: 12px;}
        ...
        <li class="nav-item">
            ...
            <div class="collapse show" id="sub-thuoc-tinh">
                <ul class="sub-menu">
                    ...
                    <li><a href="${pageContext.request.contextPath}/thuoc-tinh/pin/hien-thi" class="nav-link-custom active-sub"><i class="fa-solid fa-battery-three-quarters me-1"></i> Thông số Pin</a></li>
...
<div class="main-content">
    <div class="mb-4">
        <h4 class="fw-bold mb-1">Quản lý thuộc tính Pin</h4>
        <p class="text-muted mb-0">Cấu hình Dung lượng Pin (Wh) và Số lượng Cell pin của sản phẩm laptop.</p>
    </div>
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card card-custom p-4">
                <h6 class="fw-bold mb-3 text-uppercase" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-plus me-2"></i>Thêm thông số Pin mới</h6>
                <form action="${pageContext.request.contextPath}/thuoc-tinh/pin/them" method="POST">
                    <div class="mb-3">
                        <label class="form-label">Tên cấu hình Pin <span class="text-danger">*</span></label>
                        <input type="text" name="tenPin" class="form-control py-2" placeholder="Ví dụ: Li-Polymer 3 Cell" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Dung lượng Pin (Wh)</label>
                        <input type="text" name="dungLuongPin" class="form-control py-2" placeholder="Ví dụ: 56Wh, 90Wh">
                    </div>
        ...
        <div class="col-md-8">
            <div class="card card-custom p-4">
                <h6 class="fw-bold text-uppercase mb-3" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-list me-2"></i>Danh sách thông số Pin</h6>
                <div class="table-responsive">
                    <table class="table table-custom table-hover align-middle text-center mb-0">
                        <thead>
                        <tr>
                            <th style="width: 60px;">STT</th>
                            <th class="text-start">Tên loại Pin</th>
                            <th>Dung lượng</th>
                            <th>Trạng thái</th>
                        ...
                        <tbody>
                        <c:forEach items="${listPin}" var="p" varStatus="status">
                            <tr>
                                <td>${status.index + 1}</td>
                                <td class="text-start fw-semibold text-dark">${p.tenPin}</td>
                                <td class="text-secondary">${empty p.dungLuongPin ? '—' : p.dungLuongPin}</td>
                                <td>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <span class="badge-inactive"><i class="fa-solid fa-circle-xmark me-1"></i> Ngừng dùng</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/pin/sua?id=${p.id}" class="btn btn-sm btn-outline-secondary px-2.5" style="border-radius: 6px;">
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/pin/xoa?id=${p.id}"
                                           class="btn btn-sm btn-outline-danger px-2.5 ${!p.trangThai ? 'disabled pointer-events-none opacity-50' : ''}"
                                           style="border-radius: 6px;" onclick="return confirm('Bạn chắc chắn muốn ngừng kích hoạt Pin này?');">
