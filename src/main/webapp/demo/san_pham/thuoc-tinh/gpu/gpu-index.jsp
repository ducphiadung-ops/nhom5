...
<jsp:include page="/demo/common/sidebar.jsp">
    ...
    <jsp:param name="activeSub"  value="gpu"/>
</jsp:include>
                        ...
                        <table class="table table-custom table-hover align-middle text-center mb-0">
                            ...
                            <c:forEach items="${listGpu}" var="gpu" varStatus="status">
                                <tr>
                                    ...
                                    <td><c:choose><c:when test="${gpu.trangThai == 1}"><span class="badge-active">Sử dụng</span></c:when><c:otherwise><span class="badge-inactive">Ngừng dùng</span></c:otherwise></c:choose></td>
                                    <c:if test="${not isNhanVien}">
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="${pageContext.request.contextPath}/thuoc-tinh/gpu/sua?id=${gpu.id}" class="btn btn-sm btn-outline-secondary" style="border-radius:6px;"><i class="fa-regular fa-pen-to-square"></i></a>
                                            <a href="${pageContext.request.contextPath}/thuoc-tinh/gpu/xoa?id=${gpu.id}" class="btn btn-sm btn-outline-danger" style="border-radius:6px;" onclick="return confirm('Ngừng kích hoạt GPU này?');"><i class="fa-regular fa-trash-can"></i></a>
                                        </div>
                                    </td>
                                    </c:if>
                                </tr>
                            </c:forEach>
                            <c:if test="${empty listGpu}"><tr><td colspan="4" class="text-center py-5 text-muted">Chưa có dữ liệu GPU nào!</td></tr></c:if>
                            </tbody>
                        </table>
                    </div>
                </div>
