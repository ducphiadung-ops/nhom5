...
<head>
    <meta charset="UTF-8">
    <title>Quản lý Card đồ họa - Skycomputer</title>
        ...
        <li class="nav-item">
            ...
            <div class="collapse show" id="sub-thuoc-tinh">
                <ul class="sub-menu">
                    ...
                    <li><a href="${pageContext.request.contextPath}/thuoc-tinh/gpu/hien-thi" class="nav-link-custom active-sub"><i class="fa-solid fa-clone me-1"></i> Card đồ họa (GPU)</a></li>
...
<div class="main-content">
    <div class="mb-4">
        <h4 class="fw-bold mb-1">Quản lý thuộc tính Card đồ họa (GPU)</h4>
        <p class="text-muted mb-0">Cấu hình danh mục các dòng card màn hình onboard hoặc card rời chuyên dụng.</p>
    </div>
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card card-custom p-4">
                <h6 class="fw-bold mb-3 text-uppercase" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-plus me-2"></i>Thêm Card đồ họa mới</h6>
                <form action="${pageContext.request.contextPath}/thuoc-tinh/gpu/them" method="POST">
                    <div class="mb-3">
                        <label class="form-label">Tên bộ xử lý đồ họa <span class="text-danger">*</span></label>
                        <input type="text" name="tenGpu" class="form-control py-2" placeholder="Ví dụ: NVIDIA RTX 4060" required>
                    </div>
        ...
        <div class="col-md-8">
            <div class="card card-custom p-4">
                <h6 class="fw-bold text-uppercase mb-3" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-list me-2"></i>Danh sách bộ xử lý đồ họa</h6>
                <div class="table-responsive">
                    <table class="table table-custom table-hover align-middle text-center mb-0">
                        <thead>
                        <tr>
                            <th style="width: 60px;">STT</th>
                            <th class="text-start">Tên Card màn hình (GPU)</th>
                            <th>Trạng thái</th>
                            <th style="width: 150px;">Thao tác</th>
                        </tr>
                        </thead>
                        <tbody>
                        <c:forEach items="${listGpu}" var="gpu" varStatus="status">
                            <tr>
                                <td class="text-secondary fw-medium">${status.index + 1}</td>
                                <td class="text-start fw-semibold text-dark">${gpu.tenGpu}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${gpu.trangThai}">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/gpu/sua?id=${gpu.id}" class="btn btn-sm btn-outline-secondary px-2.5" style="border-radius: 6px;">
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/gpu/xoa?id=${gpu.id}"
                                           class="btn btn-sm btn-outline-danger px-2.5 ${!gpu.trangThai ? 'disabled pointer-events-none opacity-50' : ''}"
                                           style="border-radius: 6px;"
                                           onclick="return confirm('Bạn chắc chắn muốn ngừng kích hoạt GPU này?');">
                        ...
                        <c:if test="${empty listGpu}">
                            <tr>
                                <td colspan="4" class="text-center py-5 text-muted">
                                    ...
                                    Chưa có dữ liệu GPU nào được khởi tạo!
                                </td>
                            </tr>
                        </c:if>
                        </tbody>
