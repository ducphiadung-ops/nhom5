...
<head>
    <meta charset="UTF-8">
    <title>Quản lý Cấu hình RAM - Skycomputer</title>
        ...
        <li class="nav-item">
            ...
            <div class="collapse show" id="sub-thuoc-tinh">
                <ul class="sub-menu">
                    ...
                    <li><a href="${pageContext.request.contextPath}/thuoc-tinh/ram/hien-thi" class="nav-link-custom active-sub"><i class="fa-solid fa-memory me-1"></i> Cấu hình RAM</a></li>
...
<div class="main-content">
    <div class="mb-4">
        <h4 class="fw-bold mb-1">Quản lý thuộc tính RAM</h4>
        <p class="text-muted mb-0">Cấu hình dung lượng và chuẩn bộ nhớ RAM cho laptop.</p>
    </div>
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card card-custom p-4">
                <h6 class="fw-bold mb-3 text-uppercase" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-plus me-2"></i>Thêm RAM mới</h6>
                <form action="${pageContext.request.contextPath}/thuoc-tinh/ram/them" method="POST">
                    <div class="mb-3">
                        <label class="form-label">Tên bộ nhớ RAM <span class="text-danger">*</span></label>
                        <input type="text" name="tenRam" class="form-control py-2" placeholder="Ví dụ: DDR4 hoặc DDR5" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Dung lượng RAM</label>
                        <input type="text" name="dungLuongRam" class="form-control py-2" placeholder="Ví dụ: 8GB, 16GB">
                    </div>
        ...
        <div class="col-md-8">
            <div class="card card-custom p-4">
                <h6 class="fw-bold text-uppercase mb-3" style="font-size: 13px; color: #475569;"><i class="fa-solid fa-list me-2"></i>Danh sách cấu hình RAM</h6>
                <div class="table-responsive">
                    <table class="table table-custom table-hover align-middle text-center mb-0">
                        <thead>
                        <tr>
                            <th style="width: 60px;">STT</th>
                            <th class="text-start">Tên chuẩn RAM</th>
                            <th>Dung lượng</th>
                            <th>Trạng thái</th>
                        ...
                        <tbody>
                        <c:forEach items="${listRam}" var="ram" varStatus="status">
                            <tr>
                                <td>${status.index + 1}</td>
                                <td class="text-start fw-semibold text-dark">${ram.tenRam}</td>
                                <td class="text-secondary">${empty ram.dungLuongRam ? '—' : ram.dungLuongRam}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${ram.trangThai}">
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/ram/sua?id=${ram.id}" class="btn btn-sm btn-outline-secondary px-2.5" style="border-radius: 6px;">
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <a href="${pageContext.request.contextPath}/thuoc-tinh/ram/xoa?id=${ram.id}"
                                           class="btn btn-sm btn-outline-danger px-2.5 ${!ram.trangThai ? 'disabled pointer-events-none opacity-50' : ''}"
                                           style="border-radius: 6px;" onclick="return confirm('Bạn chắc chắn muốn ngừng kích hoạt RAM này?');">
